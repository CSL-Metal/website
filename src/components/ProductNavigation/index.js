...
function ProductCats(props) {
    ...
    console.log(productCategories)
    return (
        productCategories.map(item => <AccordionDetails >
            <Accordion >
                <AccordionSummary
                ...
                {menuElements.filter(products => products.productCategory === item).map(item => <AccordionDetails > <Typography>
                    <Link href={`products/${item.link}`} >
                        {item.product}
                    </Link></Typography>                </AccordionDetails>)}
